...
class Graph() {
	...
	private FindDay($date){
		if (isset(array_search($date, $D))) {
			return D[$date] + 1;
		}
			$D[$date] = $countOfDates;
	...
	public addEdges($current_day, $i $current_date, $L, $is_skip_for4){
	...
	public buildGraph($S, $L, $start_day, $last_day, $numerator, $holidays, $worktime, $changes, $changes_count, $start_day4, $end_day4){
	...
	addEdge(FindDay($F),$dest,$worktime, 0);
